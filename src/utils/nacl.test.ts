...
import { nacl } from './nacl'; // https://github.com/dchest/tweetnacl-js/blob/master/test/data/box.random.js

// https://github.com/dchest/tweetnacl-js/blob/master/test/data/box.random.js
const naclBoxRandomVectors = [
  [
...
describe('nacl.box', () => {
  it.each(naclBoxRandomVectors)(
    'nacl.box %s',
    (pk, sk, message, expectedCiphertext) => {
      const nonce = new Uint8Array(nacl.box.nonceLength);
      const pk1 = dec(pk);
      const sk2 = dec(sk);
      const msgBytes = dec(message);
      const goodBox = dec(expectedCiphertext);
      const box = nacl.box.seal(msgBytes, nonce, pk1, sk2);
      expect(enc(box)).toStrictEqual(enc(goodBox));
